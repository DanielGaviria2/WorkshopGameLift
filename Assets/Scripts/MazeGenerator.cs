...
public class MazeGenerator : MonoBehaviour {
    ...
    private void GenerateMaze(int rows, int columns)
    {
        ...
        mazeRows = rows;
        mazeColumns = columns;
        board.transform.localScale = new Vector3( columns*2+1, rows * 2+1, 1);
        CreateLayout();
    }
    ...
    */
        if (wallID == 1) cScript.wallL.GetComponent<Renderer>().enabled = false;
        else if (wallID == 2) cScript.wallR.GetComponent<Renderer>().enabled = false;
        else if (wallID == 3) cScript.wallU.GetComponent<Renderer>().enabled = false;
        else if (wallID == 4) cScript.wallD.GetComponent<Renderer>().enabled = false;
    }
    ...
    public void GenerateCell(Vector2 pos, Vector2 keyPos)
    {
        ...
        newCell.cellObject = Instantiate(cellPrefab, pos*2, cellPrefab.transform.rotation);
